...
# year = "2021"
# hour = "12"
# date = datetime.strptime(day+month+year+hour,"%d%m%Y%H")


def getData(date,station):
    df = WyomingUpperAir.request_data(date,station)
    pressure = df['pressure'].values * units(df.units['pressure'])
    temp = df['temperature'].values * units(df.units['temperature'])
    dewp = df['dewpoint'].values * units(df.units['dewpoint'])
...
def plotSounding(date,station):
    # call getData
    p,t,td,u,v,hght = getData(date,station)
    ...
    theta_deficit = thetaeDiff(p,thetae)
    ...
    plt.title('{} Observed'.format(station), loc='left',fontsize=18)
...
if len(station) != 3:
    raise Exception("ERROR: Invalid station entry.")


# Date/station definition
date = datetime.utcnow()
if date.hour < 13:
    new_hour = 00
else:
    new_hour = 12
date = date.replace(hour=new_hour)


try:
    print("Trying to get data from station "+station+" at date/time: " + datetime.strftime(date,"%m/%d/%Y %H UTC")+".\n")
    plotSounding(date,station)
except:
    try:
        print("Notice: New sounding not yet available, getting previous sounding instead.")
        date = date.replace(hour=12)
        date = date - timedelta(hours=24)
        plotSounding(date,station)
    except:
        raise Exception("ERROR: Could not get sounding. Check the station ID.")